/*
# Baseline Migration: Create form_sessions table and related objects

This migration creates the complete form_sessions table structure including:
1. Table structure with all columns and data types
2. Primary key and unique constraints  
3. Indexes for performance
4. Row Level Security (RLS) policies
5. Trigger function for automatic timestamp updates
6. Database function for form session upserts

This baseline captures the existing production schema to enable proper Supabase branching.
*/
...
-- Create indexes
CREATE INDEX IF NOT EXISTS form_sessions_session_id_idx 
ON public.form_sessions USING btree (session_id);
...
-- Policy: Anonymous users can create form sessions
...
-- Policy: Anonymous users can update form sessions
...
-- Policy: Anonymous users can view form sessions
...
-- Policy: Authenticated users can insert form sessions
...
-- Policy: Authenticated users can update form sessions
...
-- Policy: Authenticated users can view form sessions
...
-- Policy: Service role can access all form sessions
...
-- Create the upsert function for form sessions
