-- Step 1: Remove the DEFAULT constraint from the environment column
-- This prevents the database from automatically setting 'staging' when NULL is provided
ALTER TABLE public.form_sessions ALTER COLUMN environment DROP DEFAULT;

-- Step 2: Update the upsert_form_session function to properly handle environment field
-- This ensures the environment value from VITE_ENVIRONMENT is used exactly as-is
