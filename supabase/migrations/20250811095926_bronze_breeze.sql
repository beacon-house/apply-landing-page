-- Step 1: Remove any existing DEFAULT constraint on environment column
ALTER TABLE public.form_sessions ALTER COLUMN environment DROP DEFAULT;

-- Step 2: Update any existing records that have 'staging' to NULL (so they can be updated with correct values)
UPDATE public.form_sessions 
SET environment = NULL 
WHERE environment = 'staging';

-- Step 3: Completely replace the upsert function to ensure environment is ALWAYS taken from frontend
...
BEGIN
  ...
  VALUES (
    p_session_id,
    p_form_data->>'environment', -- DIRECT assignment from frontend
  ...
  DO UPDATE SET
    environment = EXCLUDED.environment, -- FORCE update environment field
...
-- Step 4: Verify the column has no default
SELECT column_name, column_default 
FROM information_schema.columns 
WHERE table_name = 'form_sessions' AND column_name = 'environment';