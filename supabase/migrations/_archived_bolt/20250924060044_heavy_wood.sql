/*
  # Add Location Field to Form Sessions

  1. New Column
    - Add `location` column to `form_sessions` table to store user's current city/town/place of residence
  
  2. Updated Function
    - Update `upsert_form_session` function to handle the new location field
    - Ensures proper insertion and update logic for location data
  
  3. Performance
    - No additional indexes needed as location is not used for queries
*/

-- Add location column to form_sessions table
ALTER TABLE public.form_sessions 
ADD COLUMN IF NOT EXISTS location text;

-- Update the upsert function to handle the new location field
