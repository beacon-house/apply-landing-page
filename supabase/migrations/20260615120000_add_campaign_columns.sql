-- Migration: Add 5 missing campaign/UTM columns + update upsert RPC
-- Date: 2026-06-15
--
-- The frontend collects campaign_id, utm_adset, adset_id, ad_id, utm_placement
-- from URL params but these columns didn't exist in form_sessions, so the data
-- was silently dropped on every incremental save.  The Zoho integration (triggered
-- by Supabase changes) would then see empty campaign fields.

-- ============================================================
-- 1. Add missing columns to form_sessions
-- ============================================================
...
-- ============================================================
-- 2. Recreate upsert_form_session RPC with all 11 UTM/campaign fields
-- ============================================================
...
BEGIN
  ...
  VALUES (
    ...
    p_form_data->>'utm_id',
    p_form_data->>'campaign_id',
    p_form_data->>'utm_adset',
    p_form_data->>'adset_id',
    p_form_data->>'ad_id',
    p_form_data->>'utm_placement',
  ...
  DO UPDATE SET
    ...
    campaign_id = COALESCE(EXCLUDED.campaign_id, form_sessions.campaign_id),
    utm_adset = COALESCE(EXCLUDED.utm_adset, form_sessions.utm_adset),
    adset_id = COALESCE(EXCLUDED.adset_id, form_sessions.adset_id),
    ad_id = COALESCE(EXCLUDED.ad_id, form_sessions.ad_id),
    utm_placement = COALESCE(EXCLUDED.utm_placement, form_sessions.utm_placement),
